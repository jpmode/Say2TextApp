...
struct Say2TextWatchView: View {
    ...
    @ObservedObject var connectivityManager = WatchConnectivityManager_Watch.shared
...
//    availableModels = ["base", "base.en", "tiny", "tiny.en"], original supported models
    ...
    func saveTranscriptionToCoreData(_ transcription: String) {
        ...
        let newNote = Note(context: context)  // "Note" corresponds to your Core Data entity
    ...
    func transcribeCurrentBuffer() async throws {
        ...
        await MainActor.run {
            ...
            // Send the confirmed transcription segments to the paired iPhone
            let confirmedText = segments.map { $0.text }.joined(separator: " ")  // Combine the segments into a full transcription
            sendTranscriptionToPhone(confirmedText)
            
        }
    }
    
    
//   // Function to send the transcription to the paired iPhone
    func sendTranscriptionToPhone(_ transcription: String) {
        if WCSession.default.isReachable {
            WCSession.default.sendMessage(
                ["transcription": transcription],
                replyHandler: { response in
                    print("Transcription sent to phone successfully.")
                },
                errorHandler: { error in
                    print("Error sending transcription to phone: \(error.localizedDescription)")
                }
            )
        } else {
            print("iPhone is not reachable for sending transcription.")
        }
    }
}
